<ul class="list-group">
	<li class="list-group-item active"><?php print $lang['admin_panel_account_list']; ?></li>
...
<?php
	...
	if(isset($_GET['acc'])) {
		$a = replace('get', 'acc');
		$sql0 = mysqli_query($con, "Select count(*) from $account.account where id ='$a'");
	} else {
		$sql0 = mysqli_query($con, "Select count(*) from $account.account");
	}
...
$offset = ($p - 1) * $rowsperpage;  
		...
		echo "<th>".$lang['admin_panel_account_ban_block']."</th>";
		
	if(isset($_GET['acc'])) {
		$a = replace('get', 'acc');
		$query = mysqli_query($con, "Select * from $account.account where id ='$a'");
	} else {
		$query = mysqli_query($con, "Select * from $account.account LIMIT $offset, $rowsperpage");
	}	
